title: bzoj3065 带插入区间K小值
date: 2015-03-11 08:25:44
tags: [Data Structer]
---
很久以前就听说过的神题。
<!--more-->
我写的是替罪羊树套主席树，似乎也没有很难写~(￣▽￣)~写了不到5k。。。。
替罪羊树的精髓就是部分重建。然后重建时用到线段树合并。
被坑到的是“基于引用计数的垃圾回收”这一块。。
思想就是主席树插入时引用了以前的节点，当某个节点没有被引用（自己也算一次）时就垃圾回收。后最多只有$O(nlog^{2})$个主席树节点，所以对整棵子树垃圾回收不会影响复杂度。由于每次都是自顶向下访问，所以打引用标记时只需要在被引用的子树根上打。
然后注意时刻回收。。。每次主席树插入后就试试能不能把以前的根回收等等。。。这样才不爆内存。。
想不通的一点是插入时判断高度平衡时符号打反了反而更快（就是112行那里）。。。。可能是窝理解不深刻，求神犇解释。。。
```c++
#include <cstdio>
#include <cstring>
#include <algorithm>
